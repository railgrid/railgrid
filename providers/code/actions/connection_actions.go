...
// RegistryTokenOutput is what mint-registry-token returns. It is a pull
...
// runConnection is the executor for connection-bound actions, after the gate
// has passed and the body has been decoded. provider acts as this provider in
// the request's cluster; visible is the provider's read of the Connection.
func (s *Server) runConnection(ctx context.Context, provider dynamic.Interface, req dataplane.Request, visible *unstructured.Unstructured, raw json.RawMessage) (any, *actionwire.Error) {
	var in ConnectionInput
	if err := decodeStrict(raw, &in); err != nil {
		return nil, wireError("invalid_action_input")
	}
	conn, data, store, secretKey, err := s.resolveConnection(ctx, provider, visible, in)
	if err != nil {
...
// resolveConnection pins the caller's input against the provider's read of
// the Connection, then opens the credential Secret as the provider. The caller
// never reads that Secret: it holds the credential the action exists to avoid
// handing out. The UID is what the caller saw; a Connection deleted and
// recreated under the same name since is a different credential, and the
// action fails rather than serving it.
func (s *Server) resolveConnection(ctx context.Context, provider dynamic.Interface, visible *unstructured.Unstructured, in ConnectionInput) (*api.Connection, map[string][]byte, tenant.SecretStore, string, error) {
	...
	if provider == nil || visible == nil || in.ConnectionUID == "" || string(visible.GetUID()) != in.ConnectionUID || visible.GetDeletionTimestamp() != nil {
		return fail()
	}
	var conn api.Connection
	if runtime.DefaultUnstructuredConverter.FromUnstructured(visible.Object, &conn) != nil {
		return fail()
	}
