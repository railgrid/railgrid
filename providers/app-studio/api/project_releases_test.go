...
func TestProjectReleasesOrdersEvidenceAndComputesLiveFromProductionImages(t *testing.T) {
	...
	response, err := (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).projectReleases(context.Background(), client, persisted)
	if err != nil {
...
func TestPromoteProjectSelectedHistoricalCommitResolvesFreshArtifacts(t *testing.T) {
	...
	releases, err := (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).projectReleases(context.Background(), client, persisted)
	...
	if _, _, missingEvidenceErr := (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).promoteProjectWithSelection(context.Background(), client, identity{}, persisted, nil, nil, oldSHA, true); missingEvidenceErr == nil || !strings.Contains(missingEvidenceErr.Error(), "releaseID is required") {
		t.Fatalf("promotion without releaseID error = %v, want release evidence validation", missingEvidenceErr)
	}
	updated, response, err := (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).promoteProjectWithSelection(context.Background(), client, identity{}, persisted, nil, nil, oldSHA, true, oldReleaseID)
	if err != nil {
	...
	for _, invalid := range []string{failedSHA, strings.Repeat("9", 40), ""} {
		_, _, invalidErr := (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).promoteProjectWithSelection(context.Background(), client, identity{}, updated, nil, nil, invalid, true, oldReleaseID)
	...
	_, _, staleErr := (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).promoteProjectWithSelection(context.Background(), client, identity{}, updated, nil, nil, oldSHA, true, oldReleaseID)
...
func TestPromoteProjectSelectedHistoricalCommitRejectsPartialArtifacts(t *testing.T) {
	...
	_, _, err = (&Server{tenantWorkspaces: staticWorkspaces{"cluster-a": testWorkspace("cluster-a", "org-a", "workspace-a")}.lookup, tenantActors: defaultTestActors.lookup}).promoteProjectWithSelection(context.Background(), client, identity{}, persisted, nil, nil, commitSHA, true, "sha256:stale")
