...
// running a query is POST on the custom subresource savedviews/run,
// /clusters/{clusterID}/apis/kuery.providers.railgrid.ai/v1alpha1/savedviews/{name}/run,
// which kcp authorizes with the caller's own RBAC on "savedviews/run" for
// this name before forwarding it to the provider. There is no un-named query
// route, so there is no query a tenant can run that their RBAC does not
// cover — including the playground's, which runs a per-user SavedView the
// portal creates with the kube client.
//
