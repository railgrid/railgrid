...
func subresourceRoutes() (map[string]serve.SubresourceRoute, error) {
	...
	if len(routes) == 0 {
		return nil, fmt.Errorf("%s declares no data-plane verbs or actions; the greet verb would be unreachable", path)
	}
	log.Printf("quickstart: serving %d declared coordinate(s) as kcp custom subresources (from %s)", len(routes), path)
	return routes, nil
}
