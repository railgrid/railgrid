...
// ONLY as a kcp custom subresource, and the manifest is the table serve
// dispatches those from; without it the provider has no data plane at all, so
// this is a startup error rather than a degraded mode.
var errNoCatalogEntry = errors.New("agents: no CatalogEntry manifest found — set RAILGRID_CATALOGENTRY_FILE or bake " + catalogEntryFileName + " under RAILGRID_KCP_DIR; without it no verb can be served")
...
//
// No manifest is an error (errNoCatalogEntry): there is no other way to reach
// a verb, so a provider without the table would start and silently serve no
// data plane. An unreadable or invalid manifest is an error too, because that
// is drift rather than absence.
func subresourceRoutes() (map[string]serve.SubresourceRoute, error) {
	...
	if len(routes) == 0 {
		return nil, fmt.Errorf("agents: CatalogEntry %s declares no data-plane verbs, so no verb can be served", path)
	}
	log.Printf("agents: serving %d declared coordinate(s) as kcp custom subresources (from %s)", len(routes), path)
	return routes, nil
}
