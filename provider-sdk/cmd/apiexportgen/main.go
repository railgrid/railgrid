...
//
// Every spec.dataPlane.verbs[] entry and every spec.actions[] entry becomes one
// more spec.resources[] entry, a kcp CUSTOM SUBRESOURCE named
// "<resource>/<verb>" whose storage.virtual.reference points at the
...
//
// Every spec.dependencies[].composes[] entry also becomes a permission claim
// (identity-agnostic, like every claim), so the one composition declaration
// drives the kcp side too. Such a claim is admitted by a kcp that serves admin.kcp.io
...
// hack/generate-permission-claim-policy.mjs from the same composes[] entries.
package main
