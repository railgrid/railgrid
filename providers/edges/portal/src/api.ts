...
// ─── Service catalog ──────────────────────────────────────────────
// The service-type form schema (internal/svccatalog) ships INSIDE this bundle
// as a generated JSON asset, so the UI renders the add/configure-service form
// from data instead of a hand-maintained mirror.
//
// It used to be fetched from an unauthenticated /catalog route on the
// provider. That route was not any Pillar 2 class, and it existed only
// because the portal could not read a file it already shipped. The asset is
// regenerated by providers/edges/svccatalog_asset_test.go, which fails when
// it drifts from the Go source, and being part of main.js it is covered by
// the hub's bundle integrity pin.
...
// fetchServiceCatalog returns every service type's form descriptor. It is
// static provider metadata, not tenant-scoped, and it is already here: the
// async signature is kept so callers keep treating it as data that may one day
// come from somewhere else.
const serviceCatalog = serviceCatalogAsset as CatalogEntry[]
